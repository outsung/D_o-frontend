...
import { LoginPage, ForwardPage, DuoTitle, PointerInfo } from './style';
...
function WelcomeText() {
  ...
  useFrame(({ mouse }) => {
    const c = htmlRef.current.classList.contains('on');
    if (hover === 0) {
      if (c) htmlRef.current.classList.remove('on');
    } else if (!c) htmlRef.current.classList.add('on');

    htmlRef.current.style.left = `${mouse.x * innerWidth * 0.5 + 14}px`;
    htmlRef.current.style.bottom = `${mouse.y * innerHeight * 0.5 + 22}px`;
  });

  return (
    <>
      <HTML style={{ pointerEvents: 'none', userSelect: 'none' }}>
        <PointerInfo ref={htmlRef}>click!</PointerInfo>
      </HTML>
      ...
      <PhyString
        ...
        meshProps={{
          receiveShadow: true,
          castShadow: true,
          onPointerOver: () => setHover(hover + 1),
          onPointerOut: () => setHover(hover - 1),
        }}
      />
...
function WelcomeTextBase({ children }: { children: JSX.Element }) {
  return (
    <>
      <Canvas
        colorManagement
